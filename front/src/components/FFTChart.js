...
const FFTChart = ({ rawData, sr, width, height, compact = false }) => {
  ...
  const ChartComponent = width && height ? LineChart : ResponsiveContainer;
  ...
  return (
    <div className={compact ? "" : "space-y-4"}>
        ...
        <div style={{ width: "100%", height: height || 400 }}>
          {width && height ? (
            <LineChart
              ...
              height={height}
              data={displayData}
              margin={compact ? { top: 5, right: 10, left: 0, bottom: 5 } : { top: 10, right: 30, left: 20, bottom: 60 }}
            >
              ...
              <XAxis
                ...
                label={compact ? null : { value: 'Frequency (Hz)', position: 'insideBottom', offset: -10, style: { fontSize: '12px', fill: '#64748b', fontWeight: 600 } }}
                tick={{ fontSize: 10, fill: '#64748b' }}
                stroke="#94a3b8"
                allowDecimals={true}
              />
              <YAxis
                label={compact ? null : { value: 'Amplitude', angle: -90, position: 'insideLeft', style: { fontSize: '12px', fill: '#64748b', fontWeight: 600 } }}
                tick={{ fontSize: 10, fill: '#64748b' }}
                stroke="#94a3b8"
                domain={[0, 'auto']}
              />
              {!compact && <Tooltip content={<FFTTooltip />} />}
              {stats && (
                <ReferenceLine
                  x={stats.dominantFreq}
                  stroke="#ef4444"
                  strokeDasharray="3 3"
                  label={compact ? null : { value: `Peak: ${stats.dominantFreq.toFixed(2)} Hz`, position: 'top', fill: '#ef4444' }}
                />
              )}
              <Line
                type="monotone"
      ...
      {!compact && <MetricsPanel data={rawData} />}
    </div>
  );
};
